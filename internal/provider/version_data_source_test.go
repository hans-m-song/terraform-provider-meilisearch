...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestMatchResourceAttr("data.meilisearch_version.test", "commit_date", regexp.MustCompile(`^(unknown|\d{4}-\d{2}-\d{2}.*)$`)),
					resource.TestMatchResourceAttr("data.meilisearch_version.test", "pkg_version", regexp.MustCompile(`^\d+\.\d+\.\d+`)),
					resource.TestMatchResourceAttr("data.meilisearch_version.test", "id", regexp.MustCompile(`^\d+\.\d+\.\d+`)),
				),
			},
		},
	})
